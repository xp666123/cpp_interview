...
3. 菱形继承：多重继承掺杂隔代继承1-n-1模式，此时需要用到虚继承，例如 B，C虚拟继承于A，D再多重继承B，C，否则会出错。后面有将具体虚继承怎么做。
   ...
   注：菱形继承有两个问题：**数据冗余和二义性**，**虚继承**可以解决数据冗余的问题，而**增加域名限定符**可以解决二义性的问题。
   
   **虚继承**：一种机制，类通过虚继承指出它希望共享虚基类的状态。对给定的虚基类，无论该类在派生层次中作为虚基类出现多少次，**只继承一个共享的基类子对象**，共享基类子对象称为虚基类，虚基类用virtual声明继承即可。**中间的派生类通过虚基表指针找到虚基表，再根据偏移量即可找到虚基类**。
   1.对于虚基类的初始化是由**最后的派生类负责初始化**。在最后的派生类中不仅要对直接基类进行初始化，还要负责对虚基类初始化。
   2.程序运行时，只有**最后的派生类执行对基类的构造函数调用**，而忽略其他派生类对虚基类的构造函数调用。从而避免对基类数据成员重复初始化。
